...
describe Batch::RevertsController do
  let(:batch_revert) { create(:batch_publish) }
    ...
    describe "POST 'create'" do
      ...
      context "happy path" do
        ...
        it 'creates a batch' do
          ...
          expect(assigns[:batch]).to be_kind_of BatchRevert
      ...
      context "with duplicate pids" do
        ...
        it "creates a batch with just the unique pids" do
          expect(BatchRevert).to receive(:new).with(pids: pids.uniq) { mock_model(BatchRevert) }
      ...
      context "error path - service fault" do
        ...
        it "redirects to previous page" do
          allow_any_instance_of(BatchRevert).to receive(:save) { true }
