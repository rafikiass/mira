...
describe Batch::PurgesController do
  let(:batch_purge) { create(:batch_purge) }
    ...
    describe "POST 'create'" do
      ...
      context "happy path" do
        ...
        it 'creates a batch' do
          ...
          expect(assigns[:batch]).to be_kind_of BatchPurge
      ...
      context "with duplicate pids" do
        ...
        it "creates a batch with just the unique pids" do
          expect(BatchPurge).to receive(:new).with(pids: pids.uniq) { mock_model(BatchPurge) }
      ...
      context "error path - service fault" do
        ...
        it "redirects to previous page" do
          allow_any_instance_of(BatchPurge).to receive(:save) { true }
